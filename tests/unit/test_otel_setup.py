...
def test__instrument_frameworks_all_fail(monkeypatch, caplog):
    from src.infrastructure.telemetry import otel_setup
    monkeypatch.setitem(sys.modules, "opentelemetry.instrumentation.httpx", types.ModuleType("fail"))
    monkeypatch.setitem(sys.modules, "openinference.instrumentation.agno", types.ModuleType("fail"))
...
def test__setup_log_export_handles_exception(monkeypatch, caplog):
    from src.infrastructure.telemetry import otel_setup
    def fail(*a, **k): raise RuntimeError("fail")
    monkeypatch.setattr(otel_setup, "logging", logging)
    monkeypatch.setitem(sys.modules, "opentelemetry.sdk._logs", types.ModuleType("fail"))
    caplog.set_level(logging.WARNING)

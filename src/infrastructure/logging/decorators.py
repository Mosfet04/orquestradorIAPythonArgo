...
def _mask_value(key: str, value: Any) -> Any:
    """Mascara valores de chaves sensíveis."""
...
def _mask_dict(d: Dict[str, Any]) -> Dict[str, Any]:
    """Mascara valores sensíveis em um dicionário."""
    try:
...
def _mask_args_list(items: list) -> list:
    """Mascara valores sensíveis em uma lista de argumentos."""
    return [_mask_dict(it) if isinstance(it, dict) else it for it in items]


def _build_args_data(args: tuple, kwargs: dict, mask: bool) -> Dict[str, Any]:
    """Constrói o dicionário de argumentos mascarados."""
    raw_args = _safe_args_list(args)
    raw_kwargs = dict(kwargs)
    if mask:
        return {"args": _mask_args_list(raw_args), "kwargs": _mask_dict(raw_kwargs)}
    return {"args": raw_args, "kwargs": raw_kwargs}
...
def log_execution(logger_name: str = "execution",
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs) -> Any:
            ...
            if include_args:
                args_data = _build_args_data(args, kwargs, mask_sensitive)
